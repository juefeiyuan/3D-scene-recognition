...
def load_trained_cocurrence(yolo_output_path, cur_log='trained_co_currence.txt', Display=False):
    log_file = 'trained_co_currence.txt'
    class_paths = [x for x in Path(yolo_output_path).iterdir() if x.is_dir()]
    ...
    for cur_class in class_paths:
        ...
        if cur_log.exists():
            ...
            # Text to Numpy Array to Pandas Dataframe to CSV      
            trained_cocurrence = np.loadtxt(str(cur_log))
            trained_cocurrence = np.reshape(trained_cocurrence, (1, 21945))#there are 21945 possible cocurrences
            vector_statistics[classes[index]] =  trained_cocurrence
            vector_statistics[classes[index]]=torch.from_numpy(vector_statistics[classes[index]])
